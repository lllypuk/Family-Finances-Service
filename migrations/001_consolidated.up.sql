...
-- Счёт — кошелёк, от которого сверяют расходы с банком (план 16). name_key — нормализованное имя
-- (names.Key): уникальность без учёта регистра и пробелов по краям.
...
-- План позиции (план 18): строка живёт, пока хоть одно число ненулевое. updated_at пишет upsert.
