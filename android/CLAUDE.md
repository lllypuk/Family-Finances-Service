...
каждый неглавный экран ловит `BackHandler`-ом. Три корня (главная, операции, категории) переключает
нижняя `AppNavBar`, которую подставляет `AppRoot`, а не экраны: формы её не показывают, и уход с
категорий по вкладке помечает список и главную устаревшими так же, как «назад». DI ручной — `AppGraph.create(context)`, без Hilt и
