...
`savedCount` взводит три флага устаревания. Гарантия «без дублей» переживает смерть процесса через журнал.
...
Журнал удаляют только явные события: уход с экрана (`leave` → `abandon`), «Удалить» на плашке «Главной»,
полное сохранение, новый импорт (`deleteOthers(keep)` в `init` его модели — инвариант «один журнал» держится
...
журнал пропал бы именно тогда, когда нужен; `onCleared` только отпускает `hold`. Удалённый id store помнит до
