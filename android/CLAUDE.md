...
ответа это потерянные деньги. `recognizing = true` пишется до вызова. Модель восстанавливается по журналу,
когда `claim` вернул `null`:
...
журнал пропал бы именно тогда, когда нужен. Полное сохранение, пустой ответ и импорт без годных картинок
