...
каждый неглавный экран ловит `BackHandler`-ом. Четыре корня (главная, операции, бюджеты, капитал) переключает
нижняя `AppNavBar`, которую подставляет `AppRoot`, а не экраны: формы её не показывают. Категории правят раз в
месяц, поэтому они не вкладка, а `AppScreen.Categories(back)` без панели: вход — строка в настройках и пункт
«Управлять категориями…» в листах выбора; «назад» уходит в `back` (ключ `Saver` — последним, в нём двоеточия).
`onForm` включает `Categories`, чей `back` — форма: справочник открывается поверх неё, и черновик цел. Уход с
категорий на форму взводит `categoriesStale` (только на форму: гасят его лишь формы), а форма операции, бюджета и распознавание, прочтя его, зовут
`reloadCategories()` — он пишет только `categories`: `load()` затёр бы введённые поля.
Устаревание держат шесть флагов в `AppRoot` — `listStale`, `homeStale`, `overviewStale`, `budgetsStale`,
`netWorthStale`, `categoriesStale`: экран при возврате либо перечитывает всё (`refresh`), либо только проверяет смену дня
...
Сверка (`AppScreen.Reconciliation(month)`) — остатки счетов на конец месяца и `gap` с сервера; месяц позже
текущего не выбирается, знак остатка вводится кнопкой «±» (на цифровой клавиатуре минуса часто нет).
Тап по «было» пишет остаток за прошлый месяц: счёт, заведённый в этом, в прошлом не показан сервером, и иначе
его старт не ввести. По той же причине карточка на главной до 10-го, не найдя счетов в прошлом месяце, сверяет текущий.
«Операции месяца» открывают вкладку операций как `AppScreen.Transactions(filters, reconciliation)` — только
даты месяца, без счёта: фильтр ставится в модель списка, которая живёт всю сессию, поэтому любой уход с такого
экрана («назад», вкладка) сбрасывает его. «Закрыть разницу» открывает `TransactionEdit(prefill, back =
Reconciliation(month))`: сумма `|gap|`, `income` при `gap > 0`, дата — конец месяца, но не позже сегодня
(`gapCorrection`); после сохранения форма уходит в `back`, взводя те же `*Stale`. В ключе `Saver` поля
предзаполнения стоят перед `back`: его ключ сам содержит двоеточия и обязан остаться последним.
...
`AppScreen.Transactions` с `TransactionPeriod.RANGE` на те же даты, что ушли в `summary`, `categoryIds = setOf(id)` и
...
которым не нужна живая модель формы (`resumable`): сейчас `Overview`, `Recognize(importId)` при читаемом журнале и справочник поверх такого `Recognize`.

Настройки — вложенный хост, а не вкладка: `AppRoot` знает одну ветку
...
Значок и цвет категории рисует только `CategoryAvatar` (`ui/categories/`): строки операций, листы, справочник,
главная, «Обзор». Глиф — первый графемный кластер `icon` через `android.icu.text.BreakIterator`: `java.text` режет
ZWJ-эмодзи. Не эмодзи (пусто, старые `default`/`food`) — первая буква имени. Swatch в `CategoryEditScreen`
остаётся кружком: там 2dp-кольцо означает выбранность.
...
Клиент `0.12.0` (план 20) требует сервер `v0.9.0`: сверка перешла на `/accounts/{id}/balances/{month}` и новую
форму `/stats/reconciliation`. Клиент `0.11.0` с `v0.9.0` работает, кроме экрана сверки и
карточки сверки на «Главной»: та молча пропадает (`recorded_minor` в старой модели обязателен → `Hidden`).

Клиент `0.14.0` (план 22) требует сервер `v0.10.0` для фильтра по нескольким категориям: старый сервер берёт
первый `category_id` и молча сужает список до одной категории.
