...
нужный набор в одном стиле, а `material-icons-extended` тянет тысячи векторов ради шести. Седьмая
...
живут в отдельном `SettingsModels`, который `AppRoot` чистит при **каждой** смене страницы и при
...
Compose-тестами покрыты экраны с вводом (вход, список и форма транзакции, список и форма бюджета);
главная и категории — через ViewModel: там нет ввода, а разметка меняется чаще логики.
