...
новый не публикуется в `pending` — экран показывает «откроется после сохранения» по `waiting`. В
просмотре или на отказе новый импорт вытесняет текущий: `AppRoot` чистит store форм, `onCleared` удаляет
файлы. Невзятое предложение вытесняется следующим `offer`.
