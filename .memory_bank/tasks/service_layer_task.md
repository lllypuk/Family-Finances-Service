...
- [ ] **Изучить** `internal/application/handlers/transactions.go` - текущие операции
- [ ] **Выявить расчетные операции**:
...
- [ ] **Создать интерфейс** `TransactionService`:
  ```go
...
#### 2.3 Реализация бизнес-логики
- [ ] **Создать** `internal/services/transaction_service.go`:
  - Валидация amounts и категорий
  - Автоматическое обновление связанных бюджетов
  - Проверка лимитов перед созданием expense
  - Валидация дат и периодов
  - Обработка bulk operations
- [ ] **Интеграция с BudgetService** для автоматического обновления
