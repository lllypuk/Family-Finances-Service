...
- [ ] «Распознавание скриншотов»: журнал, таблица состояний, кто удаляет журнал и почему не `onCleared`, выход и `401`, срок жизни на чтении, два корня `ImportFiles`; абзац «гарантия только в живом процессе» переписать
- [ ] навигация: возврат запомненного экрана после бутстрапа и какие экраны запоминаются
