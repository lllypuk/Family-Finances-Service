...
- Клиент: `AppScreen.kt`, `MainActivity.kt`, `ui/home`, `ApiGraph.kt`.
...
**Files:**
- Modify: `android/core/api/…/ApiGraph.kt`, `AppScreen.kt` (+ `Saver`), `MainActivity.kt`, `strings.xml`
...
- [ ] ⚠️ до кода — макет и точка входа (кандидат — карточка на «Обзоре»)
...
- Modify: `NetWorthScreen.kt`, `AppScreen.kt` (+ `Saver`), `MainActivity.kt`; при входе с «Обзора» — `ui/home/HomeScreen.kt`

- [ ] история позиции: список снимков, правка и удаление
- [ ] график ряда `net-worth` за 12 месяцев; если план 11 к этому времени дал отрисовку ряда на «Обзоре» — она же, иначе свой Canvas
