...
- [ ] разрешать путь к шаблонам устойчиво к cwd (`runtime.Caller` или поиск `go.mod` вверх), передавать его в конфиг сервера
- [ ] перестать молча глотать ошибку инициализации веб-сервера в `http_server.go:118` — логировать всегда, а в тестовом хелпере падать через `t.Fatalf`
- [ ] добавить хелпер `LoginAs(t, ts, user)`, возвращающий cookie сессии и CSRF-токен для последующих запросов
- [ ] написать тест-страж: в тестовом сервере зарегистрированы `/login` (не 404) и CSRF-middleware (POST без токена → 403)
- [ ] прогнать существующие интеграционные тесты — убедиться, что появление CSRF их не сломало, поправить где нужно
- [ ] `make test` и `make lint` — 0 issues перед задачей 2
