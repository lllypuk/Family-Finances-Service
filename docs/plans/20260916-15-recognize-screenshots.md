...
- **Качество на реальных скриншотах** — не тестами: подкоманда `recognize` (задача 5) на скриншотах
  уведомлений Сбера, Т-Банка, Альфы и списков операций. **Порог приёмки** (задача 12): суммы и типы
...
`category` — имя из переданного списка `тип: [родитель /] имя` или `null`. Разбор: снять ограду
```` ``` ````, `json.Decoder` с `UseNumber`, лишние поля терпятся, после объекта — EOF, `items`
...
### Task 11: Документация клиента и проверка на телефоне
...
- [ ] релиз на A204SO (память `android-device-testing`): share из галереи и из уведомления банка, пикер, камера, поворот на экране проверки, второй share, `503` при выключенном плече
...
- [ ] порог качества из Testing Strategy достигнут на наборе реальных скриншотов через `recognize`; протокол — в `docs/specs/`
- [ ] **прод-ingress** (внешний репозиторий, но это условие готовности релиза, поэтому здесь, а не в Post-Completion): MR в репозиторий лендинга с `request_body` для `/api/v1/transactions/recognize` влит и выкачен, загрузка пяти картинок через `ffs.shatrov.tech` прошла — без этого релиз не считается готовым
