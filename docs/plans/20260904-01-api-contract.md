...
- [x] загрузить `docs/api/openapi.yaml` через `testhelpers.RepoRoot(t)`, собрать множество `METHOD /path` из `paths`, нормализуя `{id}` ↔ `:id`
- [x] пройти `e.Routes()` тестового сервера, взять роуты с префиксом `/api/v1` плюс `GET /health`, каждый должен быть в множестве; сообщение об ошибке — список недостающих. Обратная проверка (операция без роута) добавляется в плане 04, когда контракт реализован целиком
- [x] отдельная проверка: у каждой операции есть `operationId` и хотя бы один ответ 4xx с `$ref: Error`
- [x] `make test` — зелёный; в момент написания все существующие роуты должны попасть в spec (сверить с `http_server.go:197-246`, в том числе `POST /reports` со статусом 501 как временным)
...
- [x] раздел аутентификации: bearer-токены (A-01), таблица кодов `401/403/409/429`, ничего про cookie и CSRF
- [x] пагинация `limit/offset/total` (A-08) вместо `page/page_size`; убрать `links`
- [x] единая форма ошибки, `422` для валидации; убрать «третью» форму
- [x] деньги и даты (A-05, A-06), идемпотентный `POST` с клиентским `id` (A-07)
- [x] убрать раздел про версионирование с двумя одновременными версиями и лимиты «100 req/h анонимно» — их нет и не будет; оставить `X-Request-ID`
