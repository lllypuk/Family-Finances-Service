...
- [ ] руками на debug-сборке против `ffs.shatrov.tech` под admin: переименовать **другого**
      ...
      в списке, удалить
- [ ] под member (второй телефон): нет admin-пунктов; своя новая сессия с именем устройства
