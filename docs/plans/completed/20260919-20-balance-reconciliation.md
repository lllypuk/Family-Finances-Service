...
- Клиент `0.12.0`: экран «Сверка» — итог «остатки против операций», ввод остатков, «Операции месяца»,
  «Закрыть разницу».
- Один MR на обе стороны. Контракт ломается: у клиента 0.11.0 перестают работать экран сверки и карточка
  сверки на «Главной».
...
(`PUT` — `404`, ответ `GET` не разбирается), а карточка сверки на «Главной» молча пропадает (тот же `GET`);
остальное цело. Сразу за мержем — APK `0.12.0` на оба телефона.
