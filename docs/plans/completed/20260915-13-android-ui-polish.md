...
- [x] debug-сборка на телефоне против `ffs.shatrov.tech` (пропущено — не автоматизируется): выбранный чип и активный сегмент в
...
- [x] TalkBack: FAB, переключатель пароля, кнопка даты и значок повтора озвучиваются (пропущено — не автоматизируется)
