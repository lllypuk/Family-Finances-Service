...
`USD`, `EUR`, …), трёхбуквенный латинский код вне таблицы — как есть в верхнем регистре, прочее нераспознанное → `null` (валюта семьи);
...
- [x] распознавание уведомления и списка даёт кандидатов с суммой, типом, датой и категорией; пустой результат — не ошибка (форма ответа — фикстура `probe_list.json`, `TestNormalize_EmptyItemsIsSuccess`, `TestRecognizeAPI_SuccessWithSimilar`, `emptyResultIsNotAnError`; качество на реальных уведомлениях — Post-Completion, не автоматизируется)
- [x] строка без даты, с подставленным годом, без категории, с коротким описанием или в чужой валюте не сохраняется, пока не исправлена; похожая операция помечена, но не снята
- [x] в живом процессе обрыв после отправки не создаёт дубль: повтор тем же `draft` отвечает `200`; после смерти процесса гарантии нет — записано
- [x] `LLM_OLLAMA_HOST` пуст → `503`, остальной контракт как раньше; секретов в конфиге нет
- [x] `make fmt && make test && make lint` (0 issues); `make -C android check`; `make compose-config`; `make caddy-validate`
