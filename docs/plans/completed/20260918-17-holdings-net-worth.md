...
- [x] история позиции: список снимков, правка и удаление
  - ➕ лист снимка вынесен в `ValueEditor` (общий для капитала и истории); у записанного снимка дата заперта — её смена записала бы второй снимок; удаление — в листе, «История» — ссылка из листа капитала; иконка `Pencil` в шапке истории ведёт в форму позиции
- [x] график ряда `net-worth` за 12 месяцев — свой Canvas в шапке (экрана «Обзор» в приложении нет, переиспользовать нечего); отрицательный капитал — ось нуля внутри графика, ряд из одной точки — точка, не линия
  - ➕ ноль всегда в шкале (`chartGeometry`), ось рисуется только при отрицательных корзинах
- [x] тесты ViewModel: пагинация истории, удаление снимка обновляет `current`; Compose-тест подтверждения удаления снимка
- [x] `make -C android check`
