...
┌──────────────────────────────────┐     │ записано 12 400,00   в банке 14 300,00    │
│ Сверка                           │     ├──────────────────────────────────────────┤
│ Заведите счета, чтобы сверять   ›│     │ Наличные                     не сверено   │
└──────────────────────────────────┘     │ записано 3 250,00    в банке —            │
                                         ├──────────────────────────────────────────┤
                                         │ Без счёта                     1 250,00  › │
                                         └──────────────────────────────────────────┘
```
...
- Тап по строке счёта → операции (расходы, месяц, счёт). Тап по «в банке» → лист: сумма, заметка,
  «Удалить сверку». «Без счёта» → операции с `unassigned=true`; строки нет, если там `0`.
- Разница — словами, без знака: `diff > 0` — «не записано 1 900,00», `diff < 0` — «лишнее 1 900,00», оба
  цветом ошибки; `0` — «✓ сошлось».
- Архивный счёт — приглушённым текстом; сверку ему вводить можно.
...
- [x] репозиторий сверок: `Upsert` (`ON CONFLICT(account_id, month) DO UPDATE SET …, updated_at = CURRENT_TIMESTAMP`), `Delete`, `ListByMonth`
- [x] репозиторий операций, рядом с `GetTotalsByMonth` (:839), без `familyID`, как соседи: `RecordedByAccount(ctx, from, to)` — один `SELECT account_id, SUM(amount_minor) … WHERE family_id = ? AND type = 'expense' AND date >= ? AND date <= ? GROUP BY account_id` без `JOIN accounts` (внутренний JOIN потерял бы операции без счёта); `NULL`-группа сканируется nullable-типом и даёт `unassigned`, её отсутствие — 0; сверки семьи выбираются через `accounts.family_id`
- [x] сервис: `Put` (счёт существует, сумма `0 … MaxAmount`), `Delete`, `Summary(month)` — от `List(true)` всех счетов, затем суммы и сверки; архивный входит только с расходом или строкой сверки в месяце (нулевая сверка считается)
- [x] `ReconciliationHandler` (`handlers/reconciliations.go`) владеет всеми тремя маршрутами; `GET /stats/reconciliation` регистрируется в группе `stats`, считает `ReconciliationService.Summary` — `StatsHandler` и `stats_dto.go` не трогаются; месяц по умолчанию — из `family.Location()`
- [x] `FamilyRepository.HasMonetaryData(ctx)` вместо `hasTransactions` в `family_service`; конструктор `NewFamilyService` при этом теряет `TransactionRepository`, если тот больше ни для чего не нужен (`bootstrap.go:110`, `container.go`)
- [x] спека: `putReconciliation`, `deleteReconciliation` — тег `accounts`, `getReconciliationStats` — тег `stats`; схемы `ReconciliationRequest`, `Reconciliation` + `ReconciliationOk` (ответ `PUT`), `ReconciliationStats` + `ReconciliationStatsOk`, `ReconciliationRow`, соответствующие `components/responses`
- [x] тесты сервиса: только расходы, границы месяца (1-е и последнее число входят, соседние нет), `unassigned`, `diff` отрицательный, `null` без сверки, архивный счёт
- [x] интеграция: без токена `401`, `member` — `200`; upsert дважды → одна запись и свежий `updated_at`; `PUT` без `note` очищает заметку; неизвестный счёт → `404`; дописанная операция меняет `diff` без нового `PUT`, `DELETE` счёта со сверкой → `409`, `month=2026-13` → `422`, смена валюты при сверке без операций → `409 CURRENCY_LOCKED`
- ➕ `updated_at` пишет Go (`time.Now().UTC()`, `excluded.updated_at`), а не `CURRENT_TIMESTAMP`: секундная точность не отличила бы два upsert подряд; `RecordedByAccount` — в интерфейсе `TransactionRepository`; `ReconciliationRow.account` — полная схема `Account`; коды `404`: `ACCOUNT_NOT_FOUND` и новый `RECONCILIATION_NOT_FOUND`; «только расходы» и границы месяца проверяет тест репозитория (`recorded_by_account_test.go`), сервис на моках — что границы передаются
- [x] `make fmt && make test && make lint`; `make -C android check`
...
- ➕ последний счёт — `LastAccountStore` (`PrefsLastAccountStore`, приватный SharedPreferences, в `AppGraph`), а не DataStore: ключ один и читается синхронно при сборке формы; пишется после `POST`, в том числе «без счёта»; в распознавании выбор запирается после первой записанной строки

- [x] необязательное поле «Счёт»; новая операция получает последний выбранный счёт (DataStore), если он не архивный
- [x] правка: уже привязанный архивный счёт показывается, но в листе выбора его нет; пункт «Без счёта» в листе шлёт `clear_account: true`
- [x] фильтр по счёту в списке операций
- [x] распознавание: один выбор счёта на пачку, уходит в каждый `POST /transactions`
- [x] тесты ViewModel: подстановка последнего счёта, сохранение без счёта, отвязка, правка операции с архивным счётом (показан, в листе его нет), фильтр, пачка со счётом
- [x] `make -C android check`
...
- [x] `CLAUDE.md` «Current direction»: план 16, версии
- [x] перенести план в `docs/plans/completed/`

## Post-Completion

**Figma:** экран «Сверка» и карточка на «Главной» — в файл дизайна после возврата лимита MCP или локальным
плагином.
