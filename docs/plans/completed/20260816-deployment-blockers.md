...
- [x] убрать ссылку «Создать семейный аккаунт» с `/login` — регистрация идёт по инвайтам
- [x] убрать пункт «Регистрация» из `nav.html` и `dashboard.html`
- [x] удалить осиротевший `pages/register.html` (проверено: `layouts/auth.html:41-42` ветвится только на `login_content`/`setup_content`, удаление безопасно)
- [x] написать тест: страница `/login` не содержит ссылок на несуществующие маршруты
- [x] `make test` и `make lint` — 0 issues перед задачей 17

ℹ️ Вместо ссылки на `/register` на странице входа осталась поясняющая строка
«Новые участники присоединяются по приглашению администратора семьи» — пустой
блок `auth-link` выглядел бы обрывом, а объяснение закрывает вопрос «как завести
второго пользователя» прямо там, где он возникает. В `nav.html` и
`dashboard.html` оставшийся «Вход» получил `role="button"` от удалённого пункта
— иначе в шапке анонимного посетителя не остаётся ни одной заметной кнопки.

ℹ️ Тест — ➕ `tests/integration/login_links_test.go` (`TestLoginPage_NoDeadLinks`):
он не зашивает список «плохих» ссылок, а вытаскивает со страницы **все** локальные
`href`/`src` и сверяет каждый с реально зарегистрированными GET-маршрутами Echo
(`Echo().Routes()`, шаблоны `:param`/`*` компилируются в регулярки). Так поймается
любая следующая мёртвая ссылка, а не только `/register`. Красная фаза проверена
возвратом старой разметки: падают обе проверки — точечная `NotContains` и общая
«такого GET-маршрута нет».

⚠️ Замечено, но **не тронуто** (вне чек-листа задачи): `<title>` страницы входа
по-прежнему английский — `Sign In - Семейный бюджет`. Задача 15 переводила
заголовки только финансовых разделов.
...
- [x] отметить закрытые находки в `docs/specs/002-security-audit.md` и `004-deployment-readiness.md`
- [x] обновить «API Readiness» в `README.md` — API требует аутентификации и CSRF-токена
- [x] обновить `CLAUDE.md`: убрать предупреждение «группа `/api/v1` регистрируется без auth middleware», описать починенный тестовый хелпер
- [x] описать в `deploy/README.md` первый запуск через SSH-туннель и порядок работы программного клиента с CSRF
- [ ] (отложено — действие владельца репозитория: поставить тег `v0.1.0`) убедиться, что `docker.yml` опубликовал multi-arch образ
- [ ] (выполняет харнесс после завершения всех фаз) переместить план в `docs/plans/completed/`

ℹ️ **Объём вышел за 002 и 004.** Сводная таблица в `docs/specs/README.md`
перечисляет находки из всех трёх аудитов, поэтому статусы проставлены и в
`003-ui-ux-audit.md` (U-01 и U-02 закрыты, U-05 — наполовину). Оставить 003 с
формулировкой «навигация пропадает на всех списках», отметив её закрытой в
индексе, было бы прямым противоречием. В каждый документ добавлены шапка со
статусом, таблица находок и раздел «✅ Что сделано» под каждой закрытой находкой;
исходный текст находок не переписан — история должна остаться читаемой.

ℹ️ **Честно записано то, что осталось открытым**, а не только победы:
S-03 (нет rate limiting), S-04 наполовину (`internal/config.go` по-прежнему
подставляет плейсхолдеры и `Validate()` сравнивает с ними строкой, так что секрет
`123` проходит), U-03/U-04/U-06, `/budgets/alerts` → 500, отсутствие `CSRFToken`
на дашборде, английские `Sign In` / `Accept Invitation`, `POST /api/v1/reports`
→ 501. В `CLAUDE.md` под это заведён отдельный раздел «Known rough edges» —
чтобы следующий разработчик не принял их за собственную регрессию.

⚠️ **Тег `v0.1.0` не поставлен, и ставить его прямо сейчас нельзя.** Помимо того
что это внешнее действие, требующее решения владельца репозитория, в
`.github/workflows/docker.yml` шаг `docker/build-push-action@v6` задан как
`context: .` **без `file:`**, то есть ищет `./Dockerfile`, которого в репозитории
нет (Dockerfile лежит в `docker/Dockerfile`). Пуш тега запустит workflow, и
сборка упадёт на «failed to read dockerfile». Перед первым релизом нужно
добавить `file: docker/Dockerfile`; заодно учесть, что в том же workflow есть
жёсткая проверка размера образа (падение при > 50 МБ) и smoke-тест
`docker run --rm <image> -health-check` (флаг существует). Всё это записано в
[D-02](../specs/004-deployment-readiness.md#d-02), чтобы владелец действовал с
полной информацией.

ℹ️ Попутно исправлены найденные при проверке ложные утверждения в `README.md`:
ссылки на `.memory_bank/` и `docs/tasks/` (каталогов нет) и обещания «OpenAPI 3.0
спецификация на `/api/docs`» и «Postman collection» — ни маршрута, ни файлов в
репозитории нет (`grep` по `openapi|swagger|postman` — пусто). Навигация
`docs/README.md` тоже вела на несуществующие `current_task.md`,
`current_tasks.md` и `specs/feature_xyz.md`.

ℹ️ Флоу программного API-клиента для `deploy/README.md` не пересказан по коду, а
**проверен на живом сервере** (одноразовая БД, порт 8093): setup → GET `/login`
за анонимным токеном → POST `/login` → GET `/transactions` за новым токеном
(отличается от анонимного) → GET `/api/v1/categories` 200 → POST
`/api/v1/categories` с `X-Csrf-Token` 201, без него 403, анонимно 401.
Две пойманные при этом тонкости записаны в README: после входа `/login`
перестаёт отдавать форму (сработает `RedirectIfAuthenticated`), поэтому токен
надо брать с доступной пользователю страницы; и cookie за один запрос может
переписываться несколько раз — нужен cookie jar, а не ручной разбор `Set-Cookie`.
