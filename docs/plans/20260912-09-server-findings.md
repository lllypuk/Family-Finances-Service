...
- [x] `budgetPeriodsOverlap` — нестрогое сравнение
- [x] спека: у `createBudget` фраза «границы включительные, общий день — пересечение»
- [x] unit-тесты: конец = начало соседа → пересечение; конец = начало − 1 день → нет; другая категория → нет
- [x] регрессионный unit-тест на старую пару с общим днём (создана в обход проверки): переименование
      проходит, сдвиг конца второго → `ErrBudgetOverlapExists`, сдвиг начала второго на день → проходит
      ➕ `GetByPeriod` в SQL уже включительный, кандидат с общим днём попадает в выборку — запрос не менялся
- [x] `make fmt && make test && make lint` зелёные
