...
> [specs/005-api-only-redesign.md](specs/005-api-only-redesign.md); планы 01–10 и 12–21 выполнены (веб-интерфейс
...
> серией, интерфейс клиента приведён к Material 3, метрики Prometheus на служебном слушателе, распознавание
> скриншотов, счета, капитал; релизы — сервер `v0.8.0`, клиент `app-v0.10.0`; план 19 — журнал импорта и экран
> «Обзор», клиент `app-v0.11.0`; план 20 — сверка остатков на конец месяца вместо цифры банка, сервер `v0.9.0`,
> клиент `app-v0.12.0`; план 21 — светлая тема и переключатель «Система / Светлая / Тёмная», клиент `0.13.0`).
...
### ✅ Android-клиент
- **`android/`** — Kotlin/Compose, модули `:app` и `:core:api`; вход, главная, транзакции, категории,
  бюджеты; четыре корня переключает нижняя панель вкладок
- **Настройки** — профиль, смена пароля, сессии с отзывом, а у админа ещё пользователи, семья и бэкапы;
  вход с главной по иконке профиля
- **Клиент генерируется** из `api/openapi.yaml` в `android/core/api/generated` и коммитится
- **Сборка и подпись APK** — `make -C android`, подробности в `android/CLAUDE.md`
...
- `plans/*.md` - планы в работе, задача за задачей, с отметками `[x]` (сейчас пусто)
