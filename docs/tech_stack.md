...
- **Миграции**: `001_consolidated` — читаемая картина схемы, поверх неё нумерованные миграции
...
- **Файлы**: `migrations/001_consolidated.{up,down}.sql` плюс нумерованные `NNN_*.{up,down}.sql`
- **Автоматизация**: Миграции применяются при старте приложения через `golang-migrate`;
  `go run ./cmd/server migrate --to N` — ручной переход между версиями
- **Добавление изменений**: правка `001` (для свежей базы) **и** новая `NNN` (для выкаченной) —
