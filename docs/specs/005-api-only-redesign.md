...
План 08 закрыт: все восемь планов выполнены — остаются действия владельца на сервере
(DNS, `install.sh`, `setup`, cron бэкапа, тег `v0.1.0`). Следующим планом — отчёты с экспортом
в приложении (`docs/backlog.md`).
