...
CSV-экспорт отчётов — вопрос закрыт 13.09.2026: удалить. Отчёт в приложении это экран поверх
статистики, а не файл; `/reports` вместе с `ExportReport` снесены [планом 10](#планы).

### A-13. Приложение живёт в этом же репозитории

Android-клиент кладётся сюда, в `android/`, а не в отдельный репозиторий. Контракт остаётся
в единственном экземпляре — `docs/api/openapi.yaml`; вендоренной копии и файла-пина, как между
`field-engine` и `field-engine-hub`, здесь не будет.

Причина в том, чем эта пара отличается от той. Пин защищает **опубликованного** клиента: пока
приложение в сторе, сервер обязан уметь работать со старой версией контракта. Здесь клиент не
публикуется — APK ставится на два телефона владельца, — и защищать нечего. Взамен пропадает
пошлина: каждое уточнение схемы во время совместного проектирования экрана и API было бы двумя
MR в двух репозиториях плюс бамп пина.

Цена решения, принятая сознательно:

- Правка только Kotlin гоняет Go-джобы и перевыкатывает сервер тем же кодом под новым тегом.
  Правил `changes:` не заводим, пока это не начнёт мешать: прод обслуживает одну семью, лишний
  перезапуск стоит секунд, а промах в таких правилах стоит пропущенного выката.
- Зеркало на GitHub получает и код приложения: оно толкает `main` целиком.
- Каталог приложения нужно держать вне контекста сборки образа (`.dockerignore`).
- Теги: сервер выпускается `vX.Y.Z` и только ими (`.gitlab-ci.yml`, `.release-tags`), сборка
  приложения — своим пространством имён, например `app-vX.Y.Z`.

Если клиент когда-нибудь поедет в стор, разделение обойдётся фильтрацией истории каталога
`android/` в новый репозиторий; история бэкенда не переписывается. Делать это до первого
публичного релиза, а не после.
...
| 07 | [android-budgets](../plans/completed/20260911-07-android-budgets.md) | вкладка «Бюджеты»: список с фильтром и прогрессом, форма, удаление; нижняя панель вкладок |
| 08 | [android-settings](../plans/completed/20260911-08-android-settings.md) | настройки: профиль, пароль, сессии, пользователи, семья, бэкапы; `device_name` при логине |
| 09 | [server-findings](../plans/completed/20260912-09-server-findings.md) | бюджеты: `409` со своими кодами, включительное пересечение, `is_active` только как мягкое удаление; пользователи: запись и отзыв сессий одной транзакцией |
| 10 | [reports-removal-stats-monthly](../plans/completed/20260913-10-reports-removal-stats-monthly.md) | снос `/reports` и таблицы `reports` (миграция `003`), сводка на SQL-агрегатах, `GET /stats/monthly`, подкоманда `migrate --to` |

Планы 01–10 выполнены, `v0.2.0` поставлен 12.09.2026; план 10 снёс `/reports` и добавил
`/stats/monthly`, поэтому следующий серверный релиз — `v0.3.0`. Следующим планом — 11-й,
клиентский: экран «Обзор» на `summary` + `monthly` и мультивыбор операций (`docs/backlog.md`).
