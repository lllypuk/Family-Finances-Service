...
План 07 закрыт: все семь планов выполнены — остаются действия владельца на сервере
(DNS, `install.sh`, `setup`, cron бэкапа, тег `v0.1.0`). Следующим планом — отчёты, профиль,
пользователи, сессии и бэкапы в приложении (`docs/backlog.md`).
