...
Планы 01–10 и 12–14 выполнены: сервер прошёл `v0.3.0` (план 10) и `v0.4.0` (план 12), клиент — `app-v0.6.0`
(план 13, контракт не трогал). План 14 контракт тоже не менял, поэтому его релиз — `v0.5.0`. Остаётся 11-й,
