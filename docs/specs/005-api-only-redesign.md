...
### A-14. Периодические бюджеты — серия с ленивым хвостом

Бюджет с `recurring: true` — хвост серии: `series_id` связывает инстансы, сервер сам создаёт следующий
календарный период и переносит флаг, прошлые инстансы остаются историей. Материализация ленивая, при
чтении (`GET /budgets`, `GET /stats/summary`), до «сегодня» по таймзоне семьи — на двух пользователей
фоновый планировщик стоит дороже, чем стоит: он требует своего процесса, своего лога и своей проверки
живости, а промах ленивой достройки виден сразу на экране. `spent` каждого инстанса считается из
транзакций его периода, поэтому опоздавшая материализация не портит цифры.

Правится только хвост: правка хвоста и есть «правка будущих месяцев», правка прошлого инстанса — только
его. Ни `scope` у операций, ни массовых правок: `409 BUDGET_NOT_TAIL` говорит клиенту перечитать список.
Даты членов серии неизменяемы и выровнены по календарю (`custom` не повторяется) — иначе «следующий
период» нечем вычислять. Занятый месяц (ручной бюджет той же области или с тем же именем) серия
**перешагивает**, а не останавливается на нём: одна ручная правка не должна молча убивать серию; цена —
семантика зависит от момента чтения (удаление ручного октября до продвижения достроит октябрь, после — нет).

Попутно закрыта старая гонка: занятость периода стала предикатом внутри транзакции репозитория
(`taken`), а не чтением в сервисе — иначе ручной `POST` и материализация вставили бы два бюджета на один
месяц. Дата окончания серии, «применить ко всем прошлым» и смена периода у существующего бюджета — YAGNI.
...
| 12 | [recurring-budgets](../plans/completed/20260914-12-recurring-budgets.md) | периодические бюджеты (A-14): `recurring`/`series_id` (миграция `004`), ленивая достройка серии, `409 BUDGET_NOT_TAIL`, проверка занятости в транзакции; тумблер «Повторять» в клиенте |
| 13 | [android-ui-polish](../plans/completed/20260915-13-android-ui-polish.md) | UI-аудит клиента: роли `secondaryContainer`, сегменты вместо чипов, поле пароля с показом, FAB на «Операциях», пустые состояния с действием, месяц и «Итого» на главной, отдельный цвет дохода |
| 14 | [prometheus-metrics](../plans/completed/20260915-14-prometheus-metrics.md) | метрики `ffs_*` на служебном слушателе `METRICS_ADDR` (`GET /metrics` вне Echo), пакет `internal/metrics` без глобалов, коллекторы состояния на скрейпе, `serveAll` с возвратом ошибки старта, метки `prometheus.io/*` в прод-compose |
| 19 | [import-journal-overview](../plans/completed/20260919-19-import-journal-overview.md) | журнал импорта скриншотов в `filesDir/import` (восстановление без второго платного вызова, плашка «Продолжить» на главной), экран «Обзор» на `summary` + `monthly`, фильтр операций по диапазону дат |
| 20 | [balance-reconciliation](../plans/completed/20260919-20-balance-reconciliation.md) | сверка остатков на конец месяца вместо цифры банка: миграция `008`, `/accounts/{id}/balances/{month}`, `gap` в `/stats/reconciliation`; клиент `0.12.0` — ввод остатков, «Операции месяца», «Закрыть разницу» |
| 21 | [light-theme](../plans/completed/20260922-21-light-theme.md) | светлая палитра `LightColors`, `ThemeMode` в `SharedPreferences` и переключатель в настройках, фон окна по `values-night`; клиент `0.13.0`, контракт не тронут |

Планы 01–10 и 12–21 выполнены: сервер прошёл `v0.3.0` (план 10), `v0.4.0` (план 12), `v0.8.0` (планы 14–18) и
`v0.9.0` (план 20), клиент — `app-v0.10.0`, `app-v0.11.0` (план 19) и `app-v0.12.0` (план 20); `v0.5.0`–`v0.7.0` и `app-v0.6.0`–`app-v0.9.0` не нарезались. План 19 (клиент `0.11.0`,
контракт не трогал) закрыл половину 11-го: журнал импорта и экран «Обзор». Остаётся мультивыбор операций
(`docs/backlog.md`). План 20 (сервер `v0.9.0`, клиент `0.12.0`) ломает контракт: у клиента `0.11.0` отваливаются
экран сверки и карточка сверки на «Главной».
