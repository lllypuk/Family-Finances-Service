...
Планы 01–10, 12 и 13 выполнены, `v0.2.0` поставлен 12.09.2026; планы 10 и 12 изменили контракт после него,
поэтому следующий серверный релиз — `v0.3.0`, а клиент выходит `app-v0.6.0` строго после него (13-й контракт
не трогал). Остаётся 11-й, клиентский: экран «Обзор» на `summary` + `monthly` и мультивыбор операций
(`docs/backlog.md`).
