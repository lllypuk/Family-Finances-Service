...
`description` — подстрочный поиск. `category_id` повторяется и совпадает с любой из категорий
(сервер ≥ `v0.10.0`; старый берёт первый). Полнотекстового поиска нет. Точный список параметров каждой
