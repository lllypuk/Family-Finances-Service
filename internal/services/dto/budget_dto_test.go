...
func TestCreateBudgetDTO_AllFields(t *testing.T) {
	...
	assert.Equal(t, money.Minor(100_000), dto.AmountMinor)
...
func TestUpdateBudgetDTO_AllFields(t *testing.T) {
	name := "Updated Budget"
	amount := money.Minor(150_000)
	startDate := date.New(2024, time.February, 1)
	endDate := date.New(2024, time.February, 28)
	isActive := false
	...
	assert.Equal(t, money.Minor(150_000), *dto.AmountMinor)
...
func TestBudgetStatusDTO_AllFields(t *testing.T) {
	...
	status := BudgetStatusDTO{
		...
		DaysRemaining:         10,
		IsNearLimit:           true,
		DailyBudgetMinor:      3_333,
		DailySpentMinor:       4_000,
		ProjectedOverrunMinor: 10_000,
		Status:                BudgetStatusWarning,
	}

	assert.Equal(t, budgetID, status.BudgetID)
	assert.InDelta(t, 80.0, status.UtilizationPercent, 0.001)
