...
Проект разработан как **self-hosted решение** для развертывания на собственном сервере.

### Быстрое развертывание

```bash
# Docker + Caddy (автоматический SSL)
...
|---------|----------|-----------|
| **Docker + Caddy** | Автоматический SSL, минимум настройки | Простой |
| **Docker + Nginx** | Гибкая настройка, ручной SSL | Средний |
| **Native + Systemd** | Без Docker, максимальный контроль | Продвинутый |

### Функции безопасности

- 🔒 **TLS/SSL** с автообновлением Let's Encrypt
- 🛡️ **Rate Limiting** защита от brute-force атак
- 🔥 **UFW Firewall** с блокировкой прямого доступа к приложению
- 🚫 **Fail2ban** автоматическая блокировка IP после неудачных попыток входа
- 📋 **Security Headers** полный набор современных заголовков безопасности

### Документация по развертыванию

Подробные инструкции и спецификации задач находятся в [`docs/tasks/`](docs/tasks/):

- [Скрипт установки](docs/tasks/001-install-script.md)
- [Конфигурация reverse proxy](docs/tasks/002-reverse-proxy-config.md)
- [Production Docker Compose](docs/tasks/003-docker-compose-production.md)
- [Systemd сервисы](docs/tasks/004-systemd-service.md)
- [Скрипт обновления](docs/tasks/005-upgrade-script.md)
- [Настройка безопасности](docs/tasks/006-security-hardening.md)
...
1. Реализовать скрипты self-hosted развертывания (см. `docs/tasks/`)
2. Добавить больше сценариев интеграционного тестирования для invite flow
3. Оптимизация производительности и бенчмаркинг
4. End-to-end тестирование с agent-browser
5. Нагрузочное тестирование и стресс-тесты
